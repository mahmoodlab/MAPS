...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_csv_path', type=str, help='path to the csv file containing the cell expression data of all cases along with their labels')
    parser.add_argument('--metadata_csv_path', type=str, help='path to the csv file containing the cell expression data of all cases along with their labels')
    ...
    metadata = pd.read_csv(os.path.join(args.metadata_csv_path))

    # Merge Cytotoxic CD8 cells to CD8 cells
    data.loc[data['Annotation'] == 'Cytotoxic CD8', 'Annotation'] = 'CD8'
    data.reset_index(inplace=True)

    data['case_id'] = ['' for _ in range(data.shape[0])]
    control_cases = []
    for i, row in metadata.iterrows():
        if row['control_sample'] == 'Yes':
            control_cases.append(row['case_id'])
        data.loc[data['anonymous_identifier'] == row['image_id'], 'case_id'] = row['case_id']

    # Extracting and Saving Marker Names from the data CSV File
    non_marker_cols = ['case_id', 'anonymous_identifier', 'centroidX', 'centroidY', 'cellLabel', 'cellSize', 'Annotation', 'pointNum', 'index']
    ...
    case_ids = np.unique(data['case_id']).tolist()
    case_ids = [case_id for case_id in case_ids if case_id not in control_cases]
    df = pd.DataFrame(np.zeros((len(case_ids), len(class_names))), columns=class_names, index=case_ids)
    for case_id, row in df.iterrows():
        for class_name in class_names:
            temp_df = data[data['case_id'] == case_id]
            temp_df = temp_df[temp_df['Annotation'] == class_name]
            df.loc[case_id, class_name] = temp_df.shape[0]
    ...
    case_to_fold_mapping = {'case_id': df.index.tolist(), 'fold_id': [-1] * df.shape[0]}
    ...
    features_cols.append('case_id')
    features = data[features_cols]
    labels = np.array([class_mapping[class_name] for class_name in data['Annotation'].tolist()])
    features['cell_label'] = labels

    for i in range(fold_count):
        train_patient_list = case_to_fold_mapping[case_to_fold_mapping['fold_id'] != i]['case_id'].tolist()
        test_patient_list = case_to_fold_mapping[case_to_fold_mapping['fold_id'] == i]['case_id'].tolist()

        train_split = features[features['case_id'].isin(train_patient_list)]
        test_split = features[features['case_id'].isin(test_patient_list)]
        train_split = train_split.drop(columns=['case_id'])
        test_split = test_split.drop(columns=['case_id'])
