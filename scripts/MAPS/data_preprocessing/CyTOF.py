...
if __name__ == "__main__":
    ...
    class_mapping = {}
    for i, class_name in enumerate(np.unique(dataset['cluster.term'].tolist())):
        class_dict['class_id'].append(i)
        class_dict['class_name'].append(class_name)
        class_dict['sample_count'].append(np.sum(dataset['cluster.term'] == class_name))
    ...
    non_marker_cols = ['Unnamed: 0', 'cluster.orig', 'cluster.term']
    ...
    labels = np.array([class_mapping[class_name] for class_name in dataset['cluster.term'].tolist()])
