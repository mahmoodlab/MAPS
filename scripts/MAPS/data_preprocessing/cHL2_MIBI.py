...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_csv_path', type=str, help='path to the directory containing csv files for each image')
    parser.add_argument('--metadata_csv_path', type=str, help='path to the directory containing csv files for each image')
    ...
    for i, row in metadata.iterrows():
        if row['control_sample'] == 'Yes':
            control_samples.append(row['image_id'])
        image_list.append(row['image_id'])
    ...
    data = pd.read_csv(os.path.join(args.data_csv_path, str(image_list[0]) + '.csv'))
    ...
    for image_name in image_list:
        if image_name in control_samples:
            continue
        else:
