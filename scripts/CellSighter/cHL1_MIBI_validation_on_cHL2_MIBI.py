...
def results_reformatting(result_dir, class_names, fold_id):
    ...
    res = pd.read_csv(os.path.join(result_dir, fold_id, 'results_cHL2_MIBI.csv'))
    ...
    res_df.to_csv(os.path.join(result_dir, fold_id, 'cHL2_MIBI_results.csv'), index=False)
...
if __name__ == "__main__":
    ...
    channel_indices = pd.read_csv('datasets/cHL1_MIBI/cHL2_MIBI_to_cHL1_MIBI_channel_mapping.csv')['index'].tolist()
    os.makedirs(os.path.join(args.result_path, args.fold_id), exist_ok=True)
    writer = SummaryWriter(log_dir=os.path.join(args.base_path, 'logs', args.fold_id))
    config_path = os.path.join(args.base_path, "config_cHL2_MIBI_to_cHL1_MIBI.json")
    ...
    eval_weights = os.path.join(args.base_path, args.fold_id, "weights.pth")
    ...
    results_df.to_csv(os.path.join(args.result_path, args.fold_id, "results_cHL2_MIBI.csv"), index=False)
    class_names = pd.read_csv(os.path.join(args.base_path, 'class_names.csv'))['class_name'].tolist()
